document.addEventListener('DOMContentLoaded', () => {
    ...
    const snakesAndLadders = {
      ...
      4: 14,  // This was correct but something else is causing the issue
      9: 31,
      20: 38,
      28: 84,
      40: 59,
    ...
    function positionConnector(from, to, element) {
      if (!from || !to) return; // Exit if squares don't exist
    ...
    function generatePathOrder() {
      ...
      for (let row = 9; row >= 0; row--) {
        if (row % 2 === 1) {
          // Odd rows (starting from bottom 1-10) go left to right
    ...
    function movePlayerWithAnimation(player, spaces) {
      ...
      function moveOneStep() {
        if (stepCount < spaces && currentPathIndex + 1 < pathOrder.length) {
          ...
          // Don't check for snakes and ladders during movement, just update position
          playerPositions[player] = newPosition;
          
          // Don't trigger snake or ladder animations during the movement
          // Just update the visual position
          updatePlayerPiecesWithoutEffects();
    ...
    // Update the position of player pieces on the board without triggering effects
    function updatePlayerPiecesWithoutEffects() {
      // Remove existing player pieces
      document.querySelectorAll('.player-piece').forEach(piece => piece.remove());
      ...
      function checkSnakesAndLadders(player, position) {
        // Only check the final position after all movement steps are complete
        if (snakesAndLadders[position] !== undefined) {
          const oldPosition = position;
          const newPosition = snakesAndLadders[position];
          
          // Debug info
          console.log(`Player at position ${oldPosition}, checking snakes and ladders...`);
          ...
          setTimeout(() => {
            ...
            setTimeout(() => {
              ...
              if (oldPosition < newPosition) {
                result.textContent += ` - Climbed a ladder from ${oldPosition} to ${newPosition}!`;
              } else {
                result.textContent += ` - Slid down a snake from ${oldPosition} to ${newPosition}!`;
              }
    ...
    console.log("Snakes and Ladders mapping:", snakesAndLadders);
